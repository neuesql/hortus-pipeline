...
static void CreateMVFunc(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	...
	persistence.PersistView(db, database, unqualified_name, bind_data.query, bind_data.comment, expectations, deps);
	...
	if (!bind_data.serialized_schedule.empty()) {
		...
		persistence.PersistSchedule(db, database, unqualified_name, sched_type, sched_interval, sched_unit, sched_cron);
