...
static unique_ptr<GlobalTableFunctionState> PipelineSchedulesInit(ClientContext &context,
	...
	for (idx_t i = 0; i < databases.size(); i++) {
		...
		string table = PipelinePersistence::QualifyTable(databases[i] == "memory" ? "" : databases[i], "schedules");
		query +=
		    "SELECT view_name, schedule_type, interval_value, interval_unit, cron_expression, paused FROM " + table;
	}
...
TableFunction GetPipelineSchedulesFunction() {
	TableFunction func("pipeline_schedules", {}, PipelineSchedulesFunc, PipelineSchedulesBind, PipelineSchedulesInit);
	return func;
}
...
TableFunction GetPipelineFiresFunction() {
	TableFunction func("pipeline_fires", {}, FiresFunc, FiresBind, FiresInit);
	return func;
}
