...
static unique_ptr<GlobalTableFunctionState> PipelineExpectationLogsInit(ClientContext &context, TableFunctionInitInput &input) {
    ...
    Connection conn(db);
    state->result = conn.Query("SELECT run_id, view_name, constraint_name, total_rows, passed, failed, action FROM __pipeline__.expectation_logs ORDER BY run_id");
...
TableFunction GetPipelineExpectationLogsFunction() {
    TableFunction func("pipeline_expectation_logs", {},
                       PipelineExpectationLogsFunc, PipelineExpectationLogsBind, PipelineExpectationLogsInit);
    return func;
}
